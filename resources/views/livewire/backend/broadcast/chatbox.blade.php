    ...
    @if ($selectedConversation)
        <div class="chatbox_header">
            ...
            <div class="img_container">
                <img src="https://ui-avatars.com/api/?name={{ $selectedConversation->name }}" alt="">
        ...
        <div class="chatbox_body">
            @foreach ($messages as $message)
                <div class="msg_body  {{ auth()->id() == $message->created_by_id ? 'msg_body_me' : 'msg_body_receiver' }}"
                    style="width:80%;max-width:80%;max-width:max-content">
                    @if($message->user->id !== auth()->id()) <small>{{ $message->user->name }} </small>@endif
                    <br />
                    {{ $message->message }}
