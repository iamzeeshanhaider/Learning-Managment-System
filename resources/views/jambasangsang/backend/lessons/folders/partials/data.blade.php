<x-grid-listing view="Resource Collection" :has_list="false">

    <x-slot:stats>
        @if (isset($folders))
            {{ $folders->count() }} of {{ $folders->total() }}
        @else
            0 of 0
        @endif
    </x-slot:stats>
        ...
        <div class="tab-content py-3" id="">
            <div class="tab-pane fade show active" id="courses-grid" role="tabpanel" aria-labelledby="courses-grid-tab">
                <div class="row align-items-center">
                    ...
                    @if (!$folders->hasPages() || $folders->currentPage() === 1)
                        <x-course.folder.grid type="action" :route="route('lesson.folder.create', ['lesson' => $lesson->slug])" />
                    @endif

                    @if (isset($folders) && count($folders))
                        @foreach ($folders as $index => $folder)
                            <x-course.folder.grid :folder="$folder" :route="route('lesson.resource.index', ['lesson' => $folder->lesson->slug, 'folder' => $folder->slug])" />
                        @endforeach
                    @else
                        <div class="p-5 m-auto h-100 w-100 text-center">
                            <div class="card p-5">
                                <div class="card-title">
                                    ...
                                    @else
                                        <h5>No folders has been created yet</h5>
                                    @endif
                                </div>
                            </div>
                        </div>
    ...
    <x-slot:paginator>
        {{ isset($folders) ? $folders->links('vendor.pagination.iro-custom') : '' }}
    </x-slot:paginator>
